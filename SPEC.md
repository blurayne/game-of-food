...
| **🤖 KI-Anbieter (BYOK, Multi-Provider)** | Im ⚙️-Dialog wählbar: **Anthropic·Claude**, **OpenAI·GPT**, **Google·Gemini**, **xAI·Grok** oder **Custom (OpenAI-kompatibel)**. Pro Anbieter werden **Key, Modell und Endpoint** getrennt in `localStorage` gespeichert (`EINST.provider` / `.keys[p]` / `.models[p]` / `.endpoints[p]`; alter Einzel-`apiKey` wird zu `keys.anthropic` migriert). Modell- und Endpoint-Feld sind editierbar (Platzhalter = Default des Anbieters; z. B. `claude-opus-4-8`, `gpt-4o`, `gemini-2.0-flash`, `grok-2-latest`); bei *Custom* sind beide Pflicht. Ein Hilfe-Link führt zur Key-Seite des Anbieters (Anthropic Console, `platform.openai.com`, `aistudio.google.com/api-keys`, `console.x.ai`). **Ein `kiCall()` spricht drei Dialekte:** Anthropic `POST /v1/messages` (Structured Output, `anthropic-dangerous-direct-browser-access`), OpenAI-kompatibel `POST /chat/completions` (`Authorization: Bearer`; OpenAI/Grok erzwingen valides JSON per **`response_format: json_schema` strict** und fallen bei Ablehnung auf `json_object` zurück, Custom nutzt `json_object`) und Gemini `…/models/{model}:generateContent?key=` (`responseMimeType: application/json`). Antworten werden robust geparst (`parseJson`: Markdown-Zäune/Prosa weg, dann Reparaturpass für typografische Anführungszeichen & nachgestellte Kommas, sonst klare „kein gültiges JSON"-Meldung). Nutzung: „🤖 … nachschlagen" (unbekannte Lebensmittel → Objekt mit hergeleiteten `a`-Anteilen, 🤖-Suffix, Fakt, dauerhaft als echtes Item in `eigeneFoods` gespeichert **und sofort auf den Teller gelegt**) **und** die Lebensmittel-Übersetzung. **Hinweis:** direkter Browser-Call ⇒ der Anbieter muss **CORS** erlauben (Anthropic & Gemini tun das zuverlässig; OpenAI/Grok evtl. nicht — dann Custom-Endpoint/Proxy). |
| **🤖 Eigene (KI-)Lebensmittel verwalten** | Button im ⚙️-Dialog (mit Anzahl) öffnet einen eigenen Dialog, der alle per KI hinzugefügten Lebensmittel (`eigene`) auflistet — je Eintrag Emoji, Name, Regal/Portion/kcal und ein ✕ zum Löschen, plus „🗑 Alle löschen". Löschen entfernt das Item aus `FOODS`, `eigeneFoods`, dem Teller (`teller`/`ignoriert`) und wählt ggf. ein anderes aktives Lebensmittel |
| **🚫 Alle ignorieren** | Button in der „🍽️ Heute gegessen"-Titelzeile (nur wenn etwas auf dem Teller liegt): schaltet **alle** Teller-Einträge auf 🚫 (aus der Tagessumme ausgeschlossen) bzw. wieder zurück (Label „🚫 Alle" ⇄ „↩︎ Alle zählen"). Persistiert wie jede Teller-Änderung |
...
| **💾 Teller bleibt erhalten** | Der Teller (gegessene Mengen + 🚫-Markierungen) wird bei jeder Änderung in `localStorage` (`teller` = `{t,i}`) gespeichert und beim Start wiederhergestellt — nur für ids, deren Lebensmittel noch existiert. Ist beim Laden etwas auf dem Teller, startet die App in der **Tagesansicht**. „🗑 Zurücksetzen" leert Teller + Speicher |
