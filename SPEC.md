...
| **🌍 Sprache** | Sprachwahl im ⚙️-Dialog. **Standard: Deutsch** (fest im HTML verankert, läuft ohne Fetch). 13 weitere Sprachen aus externem [`i18n.json`](i18n.json): Bairisch, Mainzerisch, Hessisch, English, Español, Français, Italiano, Svenska, Norsk, Suomi, עברית (Hebräisch, RTL), ייִדיש (Jiddisch, RTL), Sindarin. Übersetzt ist die **UI-Chrome** (Titel, Buttons, Zähler, Einstellungen, Chart-Mitte); fehlende Strings und die **Datenschicht** (Lebensmittelnamen, „Wusstest du?"-Fakten, Segment-Funktionstexte) fallen auf Deutsch zurück. Persistiert. *Hinweis: Dialekte, Hebräisch/Jiddisch, Finnisch und das (spielerische) Sindarin sind ungeprüfte Best-Effort-Übersetzungen.* |
...
| **🤖 KI-Nachschlagen (BYOK)** | Eigener Anthropic API-Key im ⚙️-Dialog (nur `localStorage`, verlässt den Browser nur Richtung Anthropic-API). Findet die Suche nichts, erscheint ein „🤖 … mit KI nachschlagen"-Button: direkter Browser-Call auf `POST /v1/messages` (`claude-opus-4-8`, Header `anthropic-dangerous-direct-browser-access`, Structured Output per JSON-Schema mit allen Nährwert-Feldern des Datenmodells). Das Ergebnis wird in ein Lebensmittel-Objekt konvertiert (`a`-Anteile aus Trockenmasse hergeleitet, 🤖-Suffix im Namen, „Wusstest du?"-Fakt inklusive), erscheint im passenden Regal, ist such- und essbar und wird in `localStorage` (`eigeneFoods`) dauerhaft gespeichert. Ohne Key zeigt die Leer-Suche nur einen Hinweis. Fehler (401, Refusal) erscheinen im Button |
